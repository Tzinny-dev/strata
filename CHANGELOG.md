...
- Pendiente producto: `Iceberg` [`propuesta-iceberg.md`](../../propuesta-iceberg.md) (L1–L4 ✅, Catálogo REST no-go documentado §10.5), `WASM` descartado.
...
- **Catálogo Iceberg operacional L2: replay/rollback/gc** (`strata/iceberg.py` `verify_catalog_run`/`rollback_run`/`gc_catalog`, `strata/cli.py`): `strata replay <file> --verify <run> --iceberg-dir` valida además que las tablas publicadas se leen (`iceberg_scan`) y su recuento coincide con snapshots (`E082` verify previo, `E083` catálogo ilegible); `strata rollback <file> --run <id> --iceberg-dir` repunta `default` del manifest sin borrar física (inmutable, `E081` run ausente, `E083` no publicado); `strata gc <file> --iceberg-dir [--keep N] [--keep-days D] [--apply]` lista/protege `default`+últimos y borra dirs antiguos (`E084`). Recencia por mtime de dirs de run (el manifest va `sort_keys`) — `539 passed` (era `534`).
