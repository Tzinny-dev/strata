...
Protected: the last --keep runs, the run live in the views, and any
publication whose metadata export is pending. Report-only by default;
--apply drops all-or-nothing. History is never pruned, so a collected run's
rollback/replay fails loud instead of reading the wrong data.
"""
import json
